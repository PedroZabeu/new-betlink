-- Corrigir cálculo de odds médias para usar média ponderada pelo stake
-- Média ponderada é mais precisa pois considera o peso de cada aposta
...
SELECT 
  ...
  -- ODDS MÉDIA PONDERADA (corrigida)
  -- Fórmula: SUM(odds × stake) / SUM(stake)
  COALESCE(
    ROUND(
...
COMMENT ON VIEW channel_metrics_live IS 'View com métricas em tempo real dos canais. Usa média ponderada para odds (SUM(odds×stake)/SUM(stake)) para maior precisão.';
COMMENT ON COLUMN channel_metrics_live.avg_odd IS 'Média ponderada das odds pelo stake. Fórmula: SUM(odds × stake) / SUM(stake)';